IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_GetTaskMasterActivities]') AND type in (N'P', N'PC'))
BEGIN
    EXEC('CREATE PROCEDURE [dbo].[sp_GetTaskMasterActivities]
        @UnitId INT = NULL,
...
GO
ALTER PROCEDURE [dbo].[sp_GetTaskMasterActivities]
    @UnitId INT = NULL,
    @CompanyLevelId INT = NULL,
    @AssetTypes NVARCHAR(MAX),
    @TaskName NVARCHAR(100) = NULL,
...
BEGIN
    ...
    -- Main query with CTE for filtering
    ;WITH FilteredActivities AS (
        SELECT 
            ...
            -- Asset-specific filtering columns
            CASE tmt.asset_type
                WHEN 'tank' THEN (SELECT active FROM AtmTank WHERE id = tma.tag_id)
                WHEN 'piping' THEN (SELECT active FROM Piping WHERE id = tma.tag_id)
                WHEN 'pipeline' THEN (SELECT active FROM Pipeline WHERE id = tma.tag_id)
                WHEN 'pressure-vessel' THEN (SELECT active FROM PressureVessel WHERE id = tma.tag_id)
                WHEN 'psv' THEN (SELECT active FROM Psv WHERE id = tma.tag_id)
            END AS asset_active,
            CASE tmt.asset_type
                WHEN 'tank' THEN (SELECT psm FROM AtmTank WHERE id = tma.tag_id)
                WHEN 'piping' THEN (SELECT psm FROM Piping WHERE id = tma.tag_id)
                WHEN 'pipeline' THEN (SELECT psm FROM Pipeline WHERE id = tma.tag_id)
                WHEN 'pressure-vessel' THEN (SELECT psm FROM PressureVessel WHERE id = tma.tag_id)
                WHEN 'psv' THEN (SELECT psm FROM Psv WHERE id = tma.tag_id)
            END AS asset_psm
    ...
    ORDER BY
        CASE WHEN @CountOnly = 1 THEN 1 -- Skip sorting for count
             WHEN @SortBy = 'unit_id' AND @IsDesc = 0 THEN 0 END,
        CASE WHEN @SortBy = 'unit_id' AND @IsDesc = 0 THEN unit_name END ASC,
        CASE WHEN @SortBy = 'unit_id' AND @IsDesc = 0 THEN tag END ASC,
       
        CASE WHEN @SortBy = 'unit_id' AND @IsDesc = 1 THEN 0 END,
        ...
        CASE WHEN @SortBy = 'completed_date' AND @IsDesc = 0 AND @FromDashboard = 1 THEN task_name END ASC,
       
        CASE WHEN @SortBy = 'completed_date' AND @IsDesc = 1 THEN completed_date END DESC,
        CASE WHEN @SortBy = 'completed_date' AND @IsDesc = 1 AND @FromDashboard = 1 THEN task_name END ASC,
